...
function build () {
  ...
  let agartha = require('agartha').agartha;

  let page_path = agartha.path.join(agartha.cwd(),'app/modules/pages');

  let pages_directory = agartha.path.join(agartha.appDir(), 'app/pages');
  ...
  agartha._.each(pages, function (element, task) {
    ...
    if (agartha._.isString(source.callback)) {
      if (agartha.exists(source.callback)) {
        /** load JS module */
        let module = require(source.callback)
        /**
