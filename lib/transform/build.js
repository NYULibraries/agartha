...
function build () {
  ...
  var page_path = agartha.path.join(agartha.cwd(),'app/modules/pages');

  var pages_directory = agartha.path.join(agartha.appDir(), 'app/pages');

  var sources = agartha.getDirectories(pages_directory);
  ...
  agartha._.each(pages, function (element, task) {
    ...
    var source = agartha.read.json(element.files['index.json']);
    ...
    if (agartha._.isString(source.callback)) {
      if (agartha.exists(source.callback)) {
        /** load JS module */
        var module = require(source.callback);
        /**
    ...
    else agartha.emit('task.done', source);
  });
