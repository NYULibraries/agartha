...
function html(data) {
  ...
  const css = require(__dirname + '/sass').css;
    ...
    if (agartha._.isObject(assets)) {
      ...
      if (agartha._.isArray(assets.js)) {
        ...
        agartha._.each(assets.js, (js) => {
          ...
          // 1) Current module
          if (agartha.exists(agartha.path.join(current_module, js))) {
            src = agartha.path.join(current_module, js);
          }

          // 2) Base module
          else if (agartha.exists(agartha.path.join(base_module, js))) {
            src = agartha.path.join(base_module, js);
          }

          // 3) Main app shared resources, set "shared" to true
          else if (agartha.exists(agartha.path.join(agartha_shared_resources, 'javascript', js))) {
            src = agartha.path.join(agartha_shared_resources, 'javascript', js), shared = true;
          }
      ...
      if (matchWidgets && matchWidgets[0]) {
        ...
        if (agartha._.isObject(toJSON.js)) {
            agartha._.each(toJSON.js, function(js) {
              if (jsConfiguration.style == 'compressed') {
